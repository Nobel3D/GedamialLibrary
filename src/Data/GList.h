...
namespace ged
{
	namespace Data
	{
		...
		public:
			...
			{
				if(USE_DEBUG) std::cout << "# INITIALIZER LIST CTOR (" << ilist.size() << " elements)" << std::endl;
			...
			{
				if (USE_DEBUG) std::cout << "# COPY CTOR #" << std::endl;
			...
			GList(GList<T>&& move)
			{
				if (USE_DEBUG) std::cout << "# MOVE CTOR #" << std::endl;
